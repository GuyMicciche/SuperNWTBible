...
function PubOnLoad()
{
    // Handle paragraph clicks for publications
    var paragraphs = document.getElementsByTagName("p");
    for (var j = 0; j < paragraphs.length; j++) {
        paragraphs[j].onclick = OnParagraphClick;
    }
}
...
function OnVerseClick() {
    ...
    if (selectedVerse.style.backgroundColor == 'yellow') {
        ...
        //var verseNumber = selectedVerse.getElementsByClassName("vsAnchor")[0].innerText;
...
function OnParagraphClick()
{
    ...
    // Or this . . .
    if (selectedParagraph.style.backgroundColor == 'yellow') {

        // The verse number of the selected verse of type int
        var paragraphNumber = selectedParagraph.id;
        var paragraphNumberIndex;

        for (var x in paragraphCollection) {
            if (paragraphNumber == paragraphCollection[x].n) {
                paragraphNumberIndex = paragraphCollection.indexOf(paragraphCollection[x]);
            }
        }

        var len = paragraphCollection.length;

        // Remove the highlight of the unselected verse
        selectedParagraph.style.backgroundColor = 'transparent';
        selectedParagraph.removeAttribute('style');

        // Remove the unselected verse from the array
        paragraphCollection.splice(paragraphNumberIndex, 1);

        // Sort entire array numerically by verse number
        paragraphCollection.sort(function (a, b)
        {
            return a.n - b.n
        });
        ...
        selectedParagraph.style.backgroundColor = 'yellow';

        var paragraphNumber = selectedParagraph.id;
        var paragraphText = selectedParagraph.innerText.substring(paragraphNumber.length + 1);

        // Stores the selected verse in an array
        paragraphCollection.push({
            n: paragraphNumber,
            t: paragraphText
        });
        ...
        // Calling C# code
        //VerseSelection.ClearVerses();
        //for (var v in paragraphCollection) {
        //    VerseSelection.AddVerse(paragraphCollection[v].n, paragraphCollection[v].t);
        //}
    }
}

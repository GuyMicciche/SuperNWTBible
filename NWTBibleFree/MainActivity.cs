...
{
    [Activity(Label = "New World Translation of the Holy Scriptures", Icon = "@drawable/icon")]
    public class MainActivity : TabActivity
    {
        ...
        protected override void OnCreate(Bundle bundle)
        {
            ...
            //ActionbarSherlock.App.ActionBar action = this.SupportActionBar;
            //action.NavigationMode = ActionBar.NavigationModeTabs;

            //ActionbarSherlock.App.ActionBar.Tab hebrewTab = SupportActionBar.NewTab();
            //hebrewTab.SetText("Hebrew");

            //ActionbarSherlock.App.ActionBar.Tab greekTab = SupportActionBar.NewTab();
        ...
        {
            List<string> files = ThisApp.FileDownloadList();
            string nonexistentFile = "";

            for (var i = 0; i < files.Count; i++)
            {
                if (!ThisApp.EpubExists(files[i] + ".xml"))
                {
                    nonexistentFile = files[i];                    
                    break;
                };
            }

            if (!string.IsNullOrEmpty(nonexistentFile))
            {
                Console.WriteLine("This is the first file found which does not exist: " + nonexistentFile + ".xml");

                ThisApp.SuperDialog(this,
                        "WARNING",
                        "A certain file needs to be downloaded. Click Download to download it. Click Cancel to ignore.",
                        "Download",
                        "Cancel",
                        delegate { ThisApp.DownloadLanguagePack(ThisApp.Language); },
                        null).Show();
            }
            else
            {
                Console.WriteLine("All the correct files are downloaded.");
            }
        }
        ...
        public void ThisApp_LanguageChanged(object sender, App.LanguageChangedArgs e)
        {
            ...
            // Initialize Daily Text
            if (ThisApp.EpubExists("dailytext.xml"))
            {

...
namespace NWTBible.ReaderMenu
{
    public class ReaderFragment : SherlockFragment
    {
        ...
        private void LoadBibleReader()
        {
            ...
            // Clean-up and add new span
            html = Regex.Replace(html, @"(<span id='\d+')(.*?)(</a>)", delegate(Match match)
            {
                string m = match.Groups[1].Value + "><span" + match.Groups[2].Value + match.Groups[3].Value + "</span>";
            ...
            if (ThisApp.preferences.GetBoolean("dualWebviews", false))
            {
                ...
                // Clean up and add new span
                html2 = Regex.Replace(html2, @"(<span id='\d+')(.*?)(</a>)", delegate(Match match)
                {
                    string m = match.Groups[1].Value + "><span" + match.Groups[2].Value + match.Groups[3].Value + "</span>";
        ...
        public void ReloadBiblePage(int offset)
        {
            ...
            // Consider Pinyin
            TogglePinyin(!ThisApp.PinyinToggle);
        ...
        */
        ...
        public void LoadDailyTextReader(int offset)
        {
            ...
            if (ThisApp.preferences.GetBoolean("dualWebviews", false))
            {
                ...
                {
                    html2 = dailyText.Content;
                }
                else
                {
                    html2 = "<html><meta name=\"viewport\" content=\"width=320\" />\n" +
                        "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\"/>\n" +
                        "<link href=\"ui/web/dt.css\" type=\"text/css\" rel=\"stylesheet\"/>\n" +
                        "<body class=\"calibre\">\n" +
                        "<div class=\"body\">" + dailyText.Content + "</div>\n" +
        ...
        private void LoadWOLDailyText(int offset)
        {
            ...
            catch (Exception ex)
            {
                ...
                Toast.MakeText(Activity, "Sorry, unable to retreive document.", ToastLength.Long).Show();
        ...
        public bool TogglePinyin(bool pinyin)
        {
            string primaryLang = ThisApp.Language;

            // Test Primary Language
            if (primaryLang == "Chinese Simplified Pinyin")
            {
                string pinyinHtml = html;

                if (!pinyin)
                {
                    Console.WriteLine("Show Pinyin\n" + pinyinHtml);
                    webview.LoadDataWithBaseURL("file:///android_asset/", pinyinHtml, "text/html", "utf-8", null);
                ...
                {
                    // Remove pinyin         
                    pinyinHtml = Regex.Replace(pinyinHtml, @"(<p class='r1'>|<p class=r1 >)(.*?)(</p>)", delegate(Match match)
                    {
                        string m = "";

                        return m;
                    });
                    pinyinHtml = Regex.Replace(pinyinHtml, @"(<p class='c1'>|<p class=c1 >)(.*?)(</p>)", delegate(Match match)
                    {
                        string m = match.Groups[2].Value;

                        return m;
                    });
                    pinyinHtml = Regex.Replace(pinyinHtml, @"<table><tr><td>", "");
                    pinyinHtml = Regex.Replace(pinyinHtml, @"</td></tr></table>", "");
                    pinyinHtml = Regex.Replace(pinyinHtml, @"</td><td>", "");
                    pinyinHtml = Regex.Replace(pinyinHtml, @"</td><td>", "");
                    pinyinHtml = Regex.Replace(pinyinHtml, @"(<div class='n'>|<div class=n>)(.*?)(</div>)", delegate(Match match)
                    {
                        string m = "<div class='par dv'><p class='sb'>" + match.Groups[2].Value + "</p>" + match.Groups[3].Value;

                        return m;
                    }, RegexOptions.Singleline);

                    Console.WriteLine("Hide Pinyin\n" + pinyinHtml);
            ...
            if (ThisApp.preferences.GetBoolean("dualWebviews", false))
            {
                string secondLang = ThisApp.preferences.GetString("listSecondLanguage", ThisApp.Language);

                if (secondLang == "Chinese Simplified Pinyin")
                {
                    string pinyinHtml2 = html2;

                    if (!pinyin)
                    {
                        webview2.LoadDataWithBaseURL("file:///android_asset/", pinyinHtml2, "text/html", "utf-8", null);
                    ...
                    {
                        // Remove pinyin         
                        pinyinHtml2 = Regex.Replace(pinyinHtml2, @"(<p class='r1'>|<p class=r1 >)(.*?)(</p>)", delegate(Match match)
                        {
                            string m = "";

                            return m;
                        });
                        pinyinHtml2 = Regex.Replace(pinyinHtml2, @"(<p class='c1'>|<p class=c1 >)(.*?)(</p>)", delegate(Match match)
                        {
                            string m = match.Groups[2].Value;

                            return m;
                        });
                        pinyinHtml2 = Regex.Replace(pinyinHtml2, @"<table><tr><td>", "");
                        pinyinHtml2 = Regex.Replace(pinyinHtml2, @"</td></tr></table>", "");
                        pinyinHtml2 = Regex.Replace(pinyinHtml2, @"</td><td>", "");
                        pinyinHtml2 = Regex.Replace(pinyinHtml2, @"(<div class='n'>|<div class=n>)(.*?)(</div>)", delegate(Match match)
                        {
                            string m = "<div class='par dv'><p class='sb'>" + match.Groups[2].Value + "</p>" + match.Groups[3].Value;
